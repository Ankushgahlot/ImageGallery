...
const Api = () => {
  ...
  return (
    <>
      ...
      <div className="text-center mt-24 flex font-semibold justify-center items-center">
        {["Nature", "Birds", "Coding", "Food"].map((item) => (
          <div
            key={item}
            onClick={() => handleSelection(item)}
            className="cursor-pointer text-white bg-slate-800 rounded pl-3 pr-3 pt-1 pb-2 ml-8 max-[380px]:ml-6"
          >
            {item}
          </div>
        ))}
      ...
      <div className="flex flex-wrap justify-evenly items-center mt-10 mb-10">
        {error && <span className="font-semibold text-2xl text-slate-800 absolute mt-20">NOT FOUND IMAGE</span>}
        {result.map((value) => (
          <img
            key={value.id}
            className="w-64 h-80 object-cover m-2 rounded-lg"
        ...
        <div className="mb-6 flex justify-center items-center">
          {page > 1 && (
            <button
              ...
              className="text-white font-semibold bg-slate-800 pt-1 pb-1 rounded pl-4 pr-4"
            >
              Previous
            </button>
          )}
          {page < totalPages && (
            <button
              onClick={() => setPage(page + 1)}
              className="text-white font-semibold bg-slate-800 pt-1 pb-1 rounded pl-4 pr-4 ml-8"
            >
              Next Page
            </button>
          )}
